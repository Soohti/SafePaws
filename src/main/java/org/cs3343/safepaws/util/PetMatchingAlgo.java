...
public class PetMatchingAlgo implements Algorithm {
    ...
    private static double calculateEuclideanDistance(final Member user, final Pet pet) {
        ...
        double normalizedWeight = normalizeValue(pet.getWeight(), MIN_WEIGHT, MAX_WEIGHT);

        double distance = 0.0;
        distance += Math.pow(profile.getExtroversionLevel() - pet.getActivityLevel(), 2);
        distance += Math.pow(profile.getDailyActivityLevel() - pet.getActivityLevel(), 2);
        distance += Math.pow(normalizedHouseSize - normalizedWeight, 2);
        distance += Math.pow(profile.getWorkHours() - DEFAULT_WORK_HOURS, 2);
        distance += Math.pow(profile.getNumberOfFamilyMembers() - DEFAULT_FAMILY_MEMBERS, 2);
        distance += Math.pow(profile.getPreviousPetExperience() - DEFAULT_PET_EXPERIENCE, 2);
        distance += Math.pow(profile.getFinancialBudget() - DEFAULT_FINANCIAL_BUDGET, 2);
    ...
    private static double calculateWeightedCosineSimilarity(final Member user, final Pet pet) {
        ...
        double petMagnitude = Math.sqrt(Math.pow(pet.getActivityLevel() * EXTROVERSION_WEIGHT, 2)
        ...
        double dotProduct = (profile.getExtroversionLevel() 
                ...
                * EXTROVERSION_WEIGHT)
                + (profile.getDailyActivityLevel() * DAILY_ACTIVITY_WEIGHT * pet.getActivityLevel()
                        * DAILY_ACTIVITY_WEIGHT)
                + (normalizedHouseSize * HOUSE_SIZE_WEIGHT * normalizedWeight * HOUSE_SIZE_WEIGHT)
                + (profile.getWorkHours() * WORK_HOURS_WEIGHT * DEFAULT_WORK_HOURS * WORK_HOURS_WEIGHT)
                + (profile.getNumberOfFamilyMembers() * FAMILY_MEMBERS_WEIGHT * DEFAULT_FAMILY_MEMBERS
                        * FAMILY_MEMBERS_WEIGHT)
                + (profile.getPreviousPetExperience() * PET_EXPERIENCE_WEIGHT * DEFAULT_PET_EXPERIENCE
                        * PET_EXPERIENCE_WEIGHT)
                + (profile.getFinancialBudget() * FINANCIAL_BUDGET_WEIGHT * DEFAULT_FINANCIAL_BUDGET
