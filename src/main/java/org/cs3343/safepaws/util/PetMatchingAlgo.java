...
public class PetMatchingAlgo implements Algorithm {
    ...
    private static double calculateWeightedCosineSimilarity(final Member user, final Pet pet) {
        ...
        double userMagnitude = Math.sqrt(Math.pow(profile.extroversionLevel * EXTROVERSION_WEIGHT, 2)
        ...
        double petMagnitude = Math.sqrt(Math.pow(pet.activityLevel * EXTROVERSION_WEIGHT, 2)
                + Math.pow(normalizedWeight * HOUSE_SIZE_WEIGHT, 2) + Math.pow(pet.healthStatus * 1.0, 2) // Assuming
        // healthStatus
        // has
        // default
        // weight
        ...
        double dotProduct = (profile.extroversionLevel * EXTROVERSION_WEIGHT * pet.activityLevel * EXTROVERSION_WEIGHT)
                + (profile.dailyActivityLevel * DAILY_ACTIVITY_WEIGHT * pet.activityLevel * DAILY_ACTIVITY_WEIGHT)
                + (normalizedHouseSize * HOUSE_SIZE_WEIGHT * normalizedWeight * HOUSE_SIZE_WEIGHT)
                + (profile.workHours * WORK_HOURS_WEIGHT * 8 * WORK_HOURS_WEIGHT)
                + (profile.numberOfFamilyMembers * FAMILY_MEMBERS_WEIGHT * 3 * FAMILY_MEMBERS_WEIGHT)
                + (profile.previousPetExperience * PET_EXPERIENCE_WEIGHT * 5 * PET_EXPERIENCE_WEIGHT)
                + (profile.financialBudget * FINANCIAL_BUDGET_WEIGHT * 500 * FINANCIAL_BUDGET_WEIGHT);
    ...
    public static double calculateMatch(final Member user, final Pet pet) {
        ...
        double speciesSimilarity = profile.preferredSpecies.equalsIgnoreCase(pet.species) ? 1.0 : 0.0;
        double breedSimilarity = profile.preferredBreed.equalsIgnoreCase(pet.breed) ? 1.0 : 0.0;
        double genderSimilarity = profile.gender.equalsIgnoreCase(pet.gender) ? 1.0 : 0.0;
        ...
        return (normalizedEuclideanDistance * weightEuclidean) + (normalizedCosineSimilarity * weightCosine)
